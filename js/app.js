...
function bindEvents() {
  ...
  document.getElementById("exportCsv").addEventListener("click", () =>
    exportCsv(state.filteredModel.agents)
  );

  document.getElementById("exportExcel").addEventListener("click", () =>
    exportExcel(state.filteredModel.agents)
  );

  document.getElementById("exportPdf").addEventListener("click", () =>
    exportPdf(state.filteredModel.agents, state.filteredModel.summary)
  );
...
async function handleUpload(event) {
  ...
  try {
    setBanner(
      "loading",
      "Reading report",
      "The interaction report is being validated and processed."
    );
    ...
    state.sheetSources = {
      ...state.sheetSources,
      ...(payload.sheetSources || {})
    };

    rebuildModel();

    const sourceLabel =
      payload.mode === "daily-kpi-workbook"
        ? "Daily KPI workbook"
        : "interaction report";
    ...
    setBanner(
      "success",
      "Report loaded",
      `${state.interactions.length.toLocaleString()} interaction rows processed from ${sourceLabel}: ${file.name}.`
    );
    ...
    setBanner(
      "error",
      "Upload failed",
      "Check that the workbook has interaction creator, interaction type, and created date columns."
    );
  }
}
...
async function refreshSheets() {
  try {
    setBanner(
      ...
      "Agent master and supplemental KPI sources are loading."
    );
    ...
    const loaded = Object.values(state.sheetSources).reduce(
    ...
    setBanner(
      ...
      `${loaded.toLocaleString()} supplemental rows are available.`
    );
    ...
    setBanner(
      "error",
      "Sheet refresh failed",
      "Review config.js for published CSV URLs or Google Sheets API credentials."
    );
  }
}
